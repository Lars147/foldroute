...
- Der Tab „Fahrten“ zeigt die letzten 20 erfolgreichen Planungen, neueste zuerst. Es sind geplante Reisen, keine automatisch erkannten abgeschlossenen Fahrten. Jede Berechnung erhält eine eigene Kennung; weitere Ergebnisse und ein Wechsel der Alternative aktualisieren denselben Eintrag. Erneutes Öffnen erzeugt keinen Eintrag. Neue erfolgreiche Berechnungen, auch Aktualisierungen, erzeugen einen neuen Eintrag.
...
- „Fahrten auf diesem Gerät speichern“ ist standardmäßig eingeschaltet. Ausschalten löscht den Verlauf; einzelne Fahrten lassen sich im Tab löschen, der gesamte Verlauf nach Bestätigung in den Einstellungen. Die vorhandene Einzelreise wird beim ersten Zugriff einmalig übernommen. Der versionierte Verlauf liegt unter `state.history` in der bestehenden IndexedDB-Datenbank; `state.last` bleibt als neuester Snapshot erhalten. Beschädigte Einträge werden einzeln entfernt. Parallele Schreibvorgänge begrenzen den Verlauf atomar auf 20 Einträge.
